...
function displayLastModified() {
    ...
    if (lastModified) {
        ...
        lastModifiedElement.textContent = `This website was updated on ${formattedDate}`;
    } else {
        lastModifiedElement.textContent = 'This website was updated on an unknown date';
    }
}

// Alternative method using file modification tracking
function trackFileModification() {
    ...
    // If no stored time exists, this is the first visit
    if (!storedTime) {
        localStorage.setItem('lastUpdateTime', currentTime.toISOString());
        return currentTime;
    }

    // Compare current page load time with stored time
    // In a real scenario, you'd want to check if the actual file content changed
    const lastUpdate = new Date(storedTime);

    // For demonstration, we'll update the timestamp each time the page is refreshed
    // In practice, you'd only update this when the actual content changes
    localStorage.setItem('lastUpdateTime', currentTime.toISOString());
...
// Optional: Update the display every minute to show relative time
